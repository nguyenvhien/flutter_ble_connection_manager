...
const Map<String, String> mockDescriptions = {
  'Smart Thermostat V2': 'Happy Path: Connects smoothly without any errors. Best for checking normal states.',
  'Fitness Tracker Pro': 'Connection Timeout: Simulates a device that fails to respond, throwing a timeout error during connection.',
  'Bluetooth Speaker X1': 'Setup Failed: Connects successfully but fails during the setup callback (e.g., unsupported firmware).',
  'Heart Rate Monitor': 'Auto-Reconnect Success: Drops connection after 3s, then successfully reconnects automatically.',
  'Smart Lock Alpha': 'Auto-Reconnect Failure: Drops connection after 3s, fails to reconnect after 3 attempts, and gives up.',
};
...
class MockBleConnectionManager implements BleConnectionManager {
  ...
  final StreamController<BleConnectionState> _stateCtrl = StreamController<BleConnectionState>.broadcast(sync: true);
  final StreamController<BleLifecycleEvent> _eventCtrl = StreamController<BleLifecycleEvent>.broadcast(sync: true);
  ...
  Future<void> connect() async {
    ...
    if (device.advName == 'Fitness Tracker Pro') { // Timeout Case
      _eventCtrl.add(ConnectionFailed(
    ...
    if (device.advName == 'Bluetooth Speaker X1') { // Setup Failed Case
      _eventCtrl.add(ConnectionFailed(
      ...
      Future.delayed(const Duration(seconds: 3), () async {
        if (_state == BleConnectionState.ready && !_disposed) {
          ...
          for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            if (attempt < 3) {
              _eventCtrl.add(RetryScheduled(attemptNumber: attempt + 1, delay: const Duration(seconds: 2)));
              await Future.delayed(const Duration(seconds: 2));
              if (_disposed) return;
            }
          }
